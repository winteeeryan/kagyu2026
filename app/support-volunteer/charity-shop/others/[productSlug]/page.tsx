...
export function generateStaticParams() {
  const category = getCharityShopCategory("others");
...
export async function generateMetadata({
  params,
}: OtherProductPageProps): Promise<Metadata> {
  const { productSlug } = await params;
  const result = getCharityShopDirectProduct("others", productSlug);
  ...
  return {
    title: `${result.product.title} | 其他 | 義賣商店 | Kagyu`,
    description: `義賣商店其他分類：${result.product.title}。`,
  };
}

export default async function Page({ params }: OtherProductPageProps) {
  const { productSlug } = await params;
  const result = getCharityShopDirectProduct("others", productSlug);
  ...
  const productDescription =
    product.description ??
    "此商品為義賣商店單品。請購送出後，將由佛學院人工確認數量、取貨方式與結緣資訊。";
      ...
      <div className={pageStyles.productDetailShell}>
        ...
        <section className={pageStyles.productHero} aria-labelledby="product-title">
          ...
          <div className={pageStyles.productInfoPanel}>
            ...
            {product.description ? (
              <ProductDescriptionDisclosure description={productDescription} detailLabel="商品詳情" />
            ) : null}
        ...
        {relatedProducts.length > 0 ? (
          <section className={pageStyles.productCommerceBand} aria-label="推薦商品">
